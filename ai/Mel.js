module.exports = {
	...
	execute: async function (client, message, functions, set, MessageEmbed) {
		if (message.mentions.has(client.user.id) || message.cleanContent.toLowerCase().includes(client.user.username.toLowerCase()) || message.channel.type == "DM") {
			...
			//=========================================================================================================
			else if (answer.intent === "Sign Language" && (message.channel.id == "328962843800109067" || message.channel.type == "DM")) {
				...
				try {
					let embed = rows.filter(row => row.name.toLowerCase().includes(signName))
			...
			//=========================================================================================================
				...
				const serverName = answer.result[0].queryResult.parameters.fields["discord-server"].stringValue
				try {
			...
			//=========================================================================================================
			else if (answer.intent === "Sign Commands" && (message.channel.id == "328962843800109067" || message.channel.type == "DM")) {
				const rows = await data.doc.sheetsByTitle["ESL"].getRows()
			...
			//=========================================================================================================
				...
				if (allowedChannels.includes(message.channel.id) || message.channel.type == "DM") {
					...
					message.reply("Try that again in the <#328962843800109067> or in a DM!")
				}
			}
			//=========================================================================================================
			else if (answer.intent === "Meme") {
				if (message.channel.id == "333796567746084864" || message.channel.type == "DM") {
					const rows = await data.doc.sheetsByTitle["Memes"].getRows()
					const meme = rows[Math.floor(Math.random() * rows.length)].meme
					message.channel.send(meme)
				} else {
					message.reply("Try that again in the <#333796567746084864> or in a DM!")
				}
			}
			//=========================================================================================================
