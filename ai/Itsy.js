		...
		if (message.mentions.has(client.user.id) || message.cleanContent.toLowerCase().includes(client.user.username.toLowerCase()) || message.channel.type == "DM") {
			...
			if (answer) {
				...
				//=========================================================================================================
					...
					try {
						...
						const CollectionList = await axios.request({
							url: "https://api.webflow.com/sites/5ed8826082212c5dc8270931/collections",
							method: "get",
							headers: {
						...
						let blogCollection = CollectionList.data.filter(collection => collection.name === "Blog Posts")

						const BlogData = await axios.request({
							url: `https://api.webflow.com/collections/${blogCollection[0]["_id"]}/items?live=true`,
							method: "get",
							headers: {
								Authorization: process.env.WEBFLOW_ITSY,
								"accept-version": "1.0.0"
						...
						const embed = new MessageEmbed().setColor("#0099ff").setTitle(BlogData.data.items[0].name).setURL(`https://www.vrcommunitybuilders.com/post/${BlogData.data.items[0].slug}`).setDescription(BlogData.data.items[0]["post-summary"]).setImage(imageLink)
				...
				//=======================================================================================================
					...
					try {
						let communityCollection = CollectionList.data.filter(collection => collection.name === "Communities")

						const CommunitiesData = await axios.request({
							url: `https://api.webflow.com/collections/${communityCollection[0]["_id"]}/items?live=true`,
							method: "get",
							headers: {
								Authorization: process.env.WEBFLOW_ITSY,
								"accept-version": "1.0.0"
						...
						let community = CommunitiesData.data.items.filter(communities => communities.name.toLowerCase() === args.toLowerCase())

						const embed = new MessageEmbed().setColor("#0099ff").setTitle(community[0].name).setURL(`https://www.vrcommunitybuilders.com/communities/${community[0].slug}`).setDescription(community[0]["discord-description"]).setThumbnail(community[0].logo.url).setImage(community[0]["cover-image"].url)
				...
				//=========================================================================================================
					...
					try {
						let memberCollection = CollectionList.data.filter(collection => collection.name === "Team Members")

						const MemberData = await axios.request({
							url: `https://api.webflow.com/collections/${memberCollection[0]["_id"]}/items`,
							method: "get",
							headers: {
								Authorization: process.env.WEBFLOW_ITSY,
								"accept-version": "1.0.0"
						...
						let Member = MemberData.data.items.filter(member => member.name === args)
						...
						const embed = new MessageEmbed()
							...
							.setURL(`https://www.vrcommunitybuilders.com/team/${Member[0].slug}`)
